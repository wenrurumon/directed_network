...
save(rlt_p2pcp,file='model20170126/rlt_p2pcp.rda')

#path final
load(file='model20170126/rlt_p2pinp.rda')
load(file='model20170126/rlt_p2pcp.rda')

#######################################################

all.equal(names(x.input),names(rlt_p2pinp[[2]]))
...
equationj2 <- function(j,i.y,lambda=0.5){
  ...
  y.i <- lapply(rlt_p2pinp[[1]][[i.y]],function(x){
    # x$score[,1:which(x$prop>0.8)[1]]
    x$score
  })
  x.i <- lapply(do.call(c,rlt_p2pinp[[1]][i.x]),function(x){
    # x$score[,1:which(x$prop>0.8)[1],drop=F]
    x$score
  })
...
model_connectout <- function(i.y,lambda=0.5){
  ...
  modeli <- lapply(1:j.len,function(j){
    equationj2(j,i.y=i.y,lambda=0.5)
  })
...
save(rlt_p2pfinal,file='model20170126/rlt_p2p.rda')

plotnet(test2[grep('signal',rownames(test2)),grep('signal',colnames(test2))])
matrix(test2,dimnames=NULL)

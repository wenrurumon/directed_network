...
expinpath <- lapply(expinpath,scale)
Y.exp <- expinpath[-3]
...
load('expr_network_20170308.rda')
...
test2 <- function(i){
  ...
  if(!is.list(Y.sem)){
    return(list(data=Y,adj=list(eq_matrix=matrix(0,1,1))))
  }
  system.time(Y.cnif <- try(CNIF(data=Y,init.adj=(Y.sem[[1]]>=stability),max_parent=3)))
  if(!is.list(Y.cnif)){
    system.time(Y.cnif <- try(CNIF(data=Y,init.adj=(Y.sem[[1]]>=stability),max_parent=2)))
...
Y <- scale(expinpath[[3]])
system.time(Y.sem <- try(sparse_2sem(Y,lambda=.8,times=1)))
system.time(Y.cnif <- try(CNIF(data=Y,init.adj=(Y.sem[[1]]>=0),max_parent=2)))
system.time(Y.rlt <- sparse_2sem(Y,lambda=lambda,Y.fixed=Y.cnif))
plotnet(Y.rlt[[1]],'directed')
